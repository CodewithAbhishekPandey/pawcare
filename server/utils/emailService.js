...
const getHeader = () => `
  <div style="background-color: #1A3A2A; padding: 20px; text-align: center;">
    <h1 style="color: white; margin: 0; font-family: sans-serif;">🐾 PawCare</h1>
  </div>
`;
...
const getFooter = () => `
  <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #eee; text-align: center; color: #888; font-size: 12px; font-family: sans-serif;">
    PawCare &middot; Gurugram, India &middot; support@pawcare.in
  </div>
`;
...
exports.sendOrderConfirmation = async (userEmail, userName, order) => {
  try {
    ...
    const mailOptions = {
      from: `"PawCare" <${process.env.EMAIL_USER}>`,
      to: userEmail,
      subject: `✅ Order Confirmed — PawCare #${order._id.toString().slice(-8).toUpperCase()}`,
      html: `
...
exports.sendOrderProcessing = async (userEmail, userName, order, agent) => {
  try {
    const mailOptions = {
      from: `"PawCare" <${process.env.EMAIL_USER}>`,
      to: userEmail,
      subject: `📦 Your order is being prepared — PawCare`,
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; background-color: #ffffff;">
          ${getHeader()}
          <div style="padding: 20px; color: #333;">
...
exports.sendOrderDelivered = async (userEmail, userName, order) => {
  try {
    const mailOptions = {
      from: `"PawCare" <${process.env.EMAIL_USER}>`,
      to: userEmail,
      subject: `🎉 Order Delivered — PawCare`,
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; background-color: #ffffff;">
          ${getHeader()}
          <div style="padding: 20px; color: #333;">
...
exports.sendOrderCancelled = async (userEmail, userName, order, cancelledBy) => {
  try {
    const reasonText = cancelledBy === 'customer'
      ...
      : `We're sorry — your order was cancelled by our team. Reason: ${order.cancellationReason || 'No reason provided'}. Contact support@pawcare.in for help.`;

    const mailOptions = {
      from: `"PawCare" <${process.env.EMAIL_USER}>`,
      to: userEmail,
      subject: `❌ Order Cancelled — PawCare`,
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; background-color: #ffffff;">
          ${getHeader()}
          <div style="padding: 20px; color: #333;">
